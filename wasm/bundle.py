...
# The trained weights are optional: without them the neural overlay reports
# itself unavailable and everything else works. They are hashed into the build id
# along with the rest, so swapping in a retrained model busts the cache too.
model_path = docs / "model.bin"
model_bytes = model_path.read_bytes() if model_path.exists() else b""

build = hashlib.sha256(js.encode("utf-8") + wasm_bytes + model_bytes).hexdigest()[:8]
...
# The weights go in too, or the one build that cannot fetch anything would be
# the one build without the overlay. They roughly triple the file, which is a
# fair price for a single document that needs no server at all.
model_script = ""
if model_bytes:
    model_script = (
        '<script>var MINESWEEPER_MODEL_BASE64 = "'
